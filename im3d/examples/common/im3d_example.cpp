...
/******************************************************************************/
	...
	static bool InitWindow(int& _width_, int& _height_, const wchar_t* _title)
	{
		...
		if (wndclassex == 0)
		{
			...
			wc.lpszClassName = L"Im3dTestApp";
...
bool Example::init(int _width, int _height, const wchar_t* _title)
{
	g_Example = this;
	...
	/*
		char buf[MAX_PATH] = {};
		DWORD buflen;
		winAssert(buflen = GetModuleFileNameA(0, buf, MAX_PATH));
		char* pathend = strrchr(buf, (int)'\\');
		*(++pathend) = '\0';
		winAssert(SetCurrentDirectoryA(buf));
...
bool Example::update()
{
	bool ret = true;
		...
		winAssert(QueryPerformanceCounter(&m_currTime));
		auto quadPart = g_SysTimerFreq.QuadPart;
		//auto quadPart = 10000000;
		//if (quadPart <= 0)
		//{
		//	return false;
		//}
		double microseconds = (double)((g_Example->m_currTime.QuadPart - g_Example->m_prevTime.QuadPart) * 1000000ll / quadPart);
