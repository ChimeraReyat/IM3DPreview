...
static Im3d::Example example;
std::thread* mainThread;

void Im3D_Main()
{
	if (!example.init(-1, -1, L"Im3d Example"))
	{
		printf("Ya got fucked kid,,\n");
		return;
	}
...
{
	MessageBoxA(NULL, "New attach: CTRL + ALT + P\nReattach: ALT + SHIFT + P", "Attach VS to process.", MB_OK);

	mainThread = new std::thread(Im3D_Main);
}
